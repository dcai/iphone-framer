...
class iPhoneFrameGenerator:
    ...
    def create_rounded_screenshot_mask(
        self, width, height, corner_radius=40, device_size="6.9_inch"
    ):
        """Create a mask for rounded corners on the screenshot, including Dynamic Island."""
        ...
        if device_size in ["6.9_inch", "6.1_inch"]:
            ...
            island_y = self.dynamic_island_dimensions[
                "y_offset"
    ...
    def create_phone_frame(
        ...
        # Create the frame image with transparent background
        ...
        draw.rounded_rectangle(screen_area, radius=corner_radius, fill=(0, 0, 0, 255))
    ...
    def add_promotional_text(self, image, title_text, subtitle_text=""):
        ...
        shadow_color = (255, 255, 255, 64)  # Semi-transparent black shadow
    ...
    def process_screenshot(
        ...
        corner_radius = self.frame_config["corner_radius"]
        screenshot_mask = self.create_rounded_screenshot_mask(
            new_width, new_height, corner_radius, device_size
        )

        # Create a new image with rounded corners
        screenshot_rounded = Image.new("RGBA", (new_width, new_height), (0, 0, 0, 0))
        screenshot_rounded.paste(screenshot_resized, (0, 0))
        screenshot_rounded.putalpha(screenshot_mask)
        ...
        if gradient_preset and gradient_preset in self.gradient_presets:
            ...
            if background_color is None:
                background_color = (25, 25, 35, 255)  # Dark blue-gray
        ...
        final_image.paste(phone_frame, (phone_x, phone_y), phone_frame)

        # Paste the rounded screenshot
        final_image.paste(
            screenshot_rounded, (screenshot_x, screenshot_y), screenshot_rounded
        )
        ...
        if output_path.lower().endswith((".jpg", ".jpeg")):
            ...
            final_rgb.paste(final_image, mask=final_image.split()[-1])
...
def main():
    ...
    if args.bg_color:
        try:
            ...
            if len(rgb_values) == 3:
                background_color = tuple(rgb_values + [255])
        except:
